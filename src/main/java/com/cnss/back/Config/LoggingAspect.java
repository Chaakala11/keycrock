...
public class LoggingAspect {
    ...
    @Before("execution(* com.cnss.back.Service.Impl.*.*(..))")
    public void logMethodEntry(JoinPoint joinPoint) {
        String name = joinPoint.getSignature().toString();
        log.info("Begin of method :" + name + " : ");
    }
    @After("execution(* com.cnss.back.Service.Impl.*.*(..))")
    public void logMethodEnd(JoinPoint joinPoint) {
        String name = joinPoint.getSignature().toString();
        log.info("End of method :" + name + " : ");
    }
    @AfterThrowing(value= "execution(* com.cnss.back.Service.Impl.*.*(..))", throwing= "exception")
